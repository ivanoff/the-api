...
export default ({ flow, options, tablesInfo }) => {
  ...
  const swagger: Is = flow
    .reduce((acc, item) => acc.concat(item.swagger), [])
    .filter(Boolean)
    .reduce((acc, cur) => {
  ...
  for (const [tableName, t] of Object.entries({ ...tablesInfo, ...tablesInfoAdditional })) {
    ...
    for (const [field, opt] of Object.entries(t)) {
      ...
      definitions += o.references
        ? `        $ref: "#/definitions/${o.references.foreign_table_name}"\n`
