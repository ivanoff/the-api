...
import { checkToken, checkOwnerToken, checkRootToken } from './check_access';
...
interface IKoaKnexHelper {
  ctx?: any;
  table?: any;
  join?: any;
  hiddenFieldsByStatus?: any;
  forbiddenFieldsToAdd?: any;
  required?: IRequired;
  defaultWhere?: any;
  tokenRequired?: any;
  ownerRequired?: any;
...
class KoaKnexHelper {
  ...
  updateHiddenColumns() {
    const hideByStatus =
      Array.isArray(this.token?.statuses) &&
  ...
  fields(_fields, db) {
    ...
    const joinCoaleise = (f || Object.keys(this.rows))
  ...
  async get({ ctx }) {
    ...
    const { _fields, _sort, _page, _skip, _limit, _lang, _isNull, _or, ...where } = ctx.request.query;
    ...
    if (_or)
      this.res.where(function () {
        [].concat(_or).map((key) => this.orWhere(key));
      });
    ...
    this.where(
      Object.entries({ ...this.defaultWhere, ...where }).reduce((acc, [cur, val]) => ({ ...acc, [`${cur}`]: val }), {}),
    );
    this.checkDeleted();
  ...
  async getById({ ctx }) {
    ...
    const { db, tablesInfo, token } = ctx.state;
    const { id } = ctx.params;
    const { _fields, _lang, _or, ...where } = ctx.request.query;
    this.token = token;
    this.lang = _lang;
