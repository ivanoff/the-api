    ...
    tables.map(async ({ table_name }) => {
      if (db.client.constructor.name === 'Client_PG') {
        const columnInfo = await db.raw(
          'select * from information_schema.columns where table_name = ? and table_schema = current_schema()',
          table_name,
        );
        result[`${table_name}`] = columnInfo.rows.reduce((acc, cur) => ({ ...acc, [cur.column_name]: cur }), {});

        for (const key of Object.keys(result[`${table_name}`])) {
          result[`${table_name}`][`${key}`].references = references.find(
            (item) => item.table_name === table_name && item.column_name === key,
          );
        }
      } else {
