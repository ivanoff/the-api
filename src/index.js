...
class TheAPI {
  constructor({ port, skipMigrations, migrationDirs = [] } = {}) {
    ...
    this.skipMigrations = skipMigrations || `${SKIP_MIGRATIONS}` === 'true';
    this.log = (...toLog) => {
  ...
  checkToken(jwtSecret) {
    this.app.use(async (ctx, next) => {
      ...
      try {
        ...
        console.log({ secret: jwtSecret.replace(/^(.{3}).+(.{3})$/, '$1...$2') });
        console.log({ token });
        console.log(err);
  ...
  async initRoutes(flowOrigin) {
    ...
    if (!this.skipMigrations) await this.migrations(flow);
