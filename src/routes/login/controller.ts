...
async function loginTool({ ctx, login: loginOrigin, password, refresh, id: byId }: IloginTool) {
  ...
  const dataToSign = tokenFields.reduce((acc, key) => {
    acc[`${key}`] = user[`${key}`];
    return acc;
  }, {});
...
async function externalLogin({ ctx, service, profile, external_id, first_name, second_name, email }: IExternalLogin) {
  ...
  const { rows: userByServiceArr } = await db.raw(
    `SELECT * FROM users WHERE external_profiles @> '[{"provider":??,"_id":??}]'`,
    [service, _id],
  );
  ...
  if (!user) {
    ...
    await db('users')
      .where({ email })
      .update({
        external_profiles: JSON.stringify([].concat(user.external_profiles, { ...profile, _id }).filter(Boolean)),
      });
  }
...
async function register(ctx) {
  ...
  const [{ id: user_id }] = await db('users')
    .insert({
      login,
...
async function check(ctx) {
  ...
  await db('code')
    .del()
    .where('time', '>', new Date(new Date().getTime() + 1000 * 60 * expireIn));
...
async function setPassword(ctx) {
  ...
  await db('users')
    .update({ password: sha256(password + salt), salt })
    .where({ login });
