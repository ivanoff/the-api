...
import { post, patch } from '../../../../test/lib/fetch';
import TheAPI from '../../../index';
...
describe('Login', () => {
  ...
  before(async () => {
    ...
    api = new TheAPI();
    await api.up([api.extensions.errors, api.routes.loginHandler, api.routes.login]);
  });
  ...
  describe('Register', () => {
    ...
    it('status 200', async () => {
      res = await post('/register', userData);
      expect(res.status).to.eql(200);
    });

    it('status 409 for same login', async () => {
      res = await post('/register', { ...userData, email: 'wrong' });
      expect(res.status).to.eql(409);
    });

    it('status 409 for same email', async () => {
      res = await post('/register', { ...userData, login: 'wrong' });
      expect(res.status).to.eql(409);
    });

    it('status 200', async () => {
      res = await post('/login', { login, password });
      expect(res.status).to.eql(200);
    });
    ...
    it('check code with no login returns status 409', async () => {
      res = await post('/register/check', { code });
      expect(res.status).to.eql(409);
    });

    it('check code returns status 200', async () => {
      res = await post('/register/check', { login, code });
      expect(res.status).to.eql(200);
    });
  ...
  describe('Use refresh', () => {
    ...
    it('has refresh', async () => {
      const rawRes = await post('/login', { login, password });
    ...
    it('New refresh token works', async () => {
      res = await post('/login', { refresh: secondRefresh });
      expect(res.status).to.eql(200);
    });
  });
  ...
  describe('Restore password', () => {
    ...
    it('status 200 for any request', async () => {
      const rawRes = await post('/login/forgot', {});
    ...
    it('status 200 for wrong request', async () => {
      const rawRes = await post('/login/forgot', { login: 'wrong' });
      expect(rawRes.status).to.eql(200);
      res = await rawRes.json();
    });
    ...
    it('status 200', async () => {
      const rawRes = await post('/login/forgot', { login });
      expect(rawRes.status).to.eql(200);
      res = await rawRes.json();
    });
    ...
    it('set new password with wrong code', async () => {
      const rawRes = await post('/login/restore', { code: 'wrong', password: newPassword });
      expect(rawRes.status).to.eql(409);
    });

    it('set new password', async () => {
      const rawRes = await post('/login/restore', { code, password: newPassword });
      expect(rawRes.status).to.eql(200);
      res = await rawRes.json();
    });
    ...
    it('login with new password result has refresh', async () => {
      const rawRes = await post('/login', { login, password: newPassword });
      expect(rawRes.status).to.eql(200);
      res = await rawRes.json();
      expect(res).to.have.property('refresh');
    });
    ...
    it('try to restore with email 200 status code', async () => {
      const rawRes = await post('/login/forgot', { email });
      expect(rawRes.status).to.eql(200);
      res = await rawRes.json();
    });
    ...
    it('try to restore with old code', async () => {
      const rawRes = await post('/login/forgot', { email });
      res = await rawRes.json();
      const rawRes2 = await post('/login/restore', { code, password: newPassword });
  ...
  describe('Update user data', () => {
    ...
    let token;
    it('status 200', async () => {
      res = await post('/register', userData2);
      expect(res.status).to.eql(200);
    });
    ...
    it('change first name', async () => {
      res = await patch('/login', { first_name: 'aaa' }, { Authorization: `Bearer ${token}` });
      expect(res).to.have.property('ok');
    });

    it('change email', async () => {
      res = await patch('/login', { email: 'bbb' }, { Authorization: `Bearer ${token}` });
      expect(res).to.have.property('ok');
    });
  ...
  describe('Mistakes', () => {
    let res;
    it('status 404', async () => {
      res = await post('/login', { login: 'wrong', password: 'bbb' });
      expect(res.status).to.eql(404);
    });

    it('status 404', async () => {
      res = await post('/login', {});
      expect(res.status).to.eql(404);
    });

    it('status 409', async () => {
      res = await post('/register', { login: '' });
      expect(res.status).to.eql(409);
    });
  });
});
